...
function MostrarAgregarCandidato(part) {
  ...
  $('#paneles').load('Templates/EdicionCandidato.html?' + new Date().getTime(), null, function(a, b, c) {
    ...
    $('#PASO-input').prop('checked', false);
    $('#ballotage-input').prop('checked', false);
  });
}
...
function MostrarEditarCandidato(cand) {
  ...
  $('#paneles').load('Templates/EdicionCandidato.html', null, function(a, b, c) {
    ...
    var ballotage = cand.ballotage == '1' ? true : false;
    document.getElementById('name-input').value = cand.nombre;
    document.getElementById('cargo-input').value = cand.cargo;
    ...
    $('#ballotage-input').prop('checked', ballotage);
    document.getElementById('partido-input').value = cand.partido;
...
function EdicionCandidato() {
  var checkPaso = $('#PASO-input');
  var checkBallotage = $('#ballotage-input');
  var _name = document.getElementById('name-input').value;
  var _cargo = document.getElementById('cargo-input').value;
  var _ciudad = 0; //document.getElementById('ciudad-input').value;
  ...
  var _Ballotage = document.getElementById('ballotage-input').checked ? 1 : 0;
  var _ballotage = document.getElementById('ballotage-input').checked ? 1 : 0;
  ...
  if (window.nombreImagenCandidato != null) {
    if ($.inArray(window.nombreImagenCandidato.split('.')[1], ['png', 'PNG', 'jpg', 'JPG', 'jpeg', 'JPEG', 'gif', 'GIF', 'bmp', 'BMP']) != -1) {
      ...
      $('#image-input').data('uploadifive').settings.formData = {
        'name': _image
      };
      $('#image-input').data('uploadifive').settings.onUploadComplete = function(file, response, data) {
        ActionEdicionCandidato(_name, _lista, _twitter, _PASO, _ballotage, _cargo, _ciudad, _partido, _image);
...
function ActionEdicionCandidato(_name, _lista, _twitter, _PASO, _ballotage, _cargo, _ciudad, _partido, _image) {
  $('#loading').fadeIn('fast');
  $.ajax({
      method: "GET",
    ...
    .done(function(msg) {
      ...
      if (!window.editarCandidato) {
        ...
        partidos.filter(function(part) {
          return part.codigo == cand.partido.codigo;
        })[0].candidatos.push(cand);
      } else {
        partidoAnterior = partidos.filter(function(_part) {
          return _part.codigo == partidoAnteriorCodigo;
        })[0];
        candidato = partidoAnterior.candidatos.filter(function(cand) {
          return cand.codigo == window.edicionCandidatoCodigo;
        })[0];
        ...
        if (cand.partido.codigo != partidoAnteriorCodigo) {
          partidoNuevo = partidos.filter(function(_part) {
            return _part.codigo == cand.partido.codigo;
          })[0];
...
function EliminarCandidato(cand, popup) {
  ...
  $.ajax({
    ...
    .done(function(msg) {
      ...
      var candidatosPartido = partidos.filter(function(part) {
        return part.codigo == cand.partido.codigo;
      })[0].candidatos;
      candidatosPartido.splice(candidatosPartido.indexOf(candidatosPartido.filter(function(e) {
        return e.codigo == cand.codigo
      })[0]), 1);
      VerificarPropuestas();
      $('#loading').fadeOut('fast');
    });
