...
function GetCandidatos(filtro, segundoFiltro) {
  ...
  var res = candidatos;
  if (cargo != -1 && ciudad == -1)
    res = candidatos.filter(function(cand) {
      return cand.cargo.codigo == cargo;
    });
  else if (cargo != -1 && ciudad != -1)
    res = candidatos.filter(function(cand) {
      return cand.cargo.codigo == cargo && cand.ciudad.codigo == ciudad;
    });
  ...
  if (segundoFiltro != undefined)
    res = res.filter(function(cand) {
      return cand.enBallotage == segundoFiltro;
    });
    ...
    case 0:
      {
        ...
        $(container).attr('data-codigo', cand.codigo);
  			...
  			$(cont).css('margin', '3px 0 0 0');
  			$(cont).css('width', '100%');

        if(part == undefined)
          part = partidos.filter(function(part){ return part.codigo == cand.partido; })[0];
    ...
    case 5:
      {
        ...
        $(propuestasDIV).html(GetPropuestas(1, cand.codigo).filter(function(prop) {
          return prop.categorias.indexOf(ganador) > -1
        }).length);
        $(container).append(propuestasDIV);
