	...
	switch(tipo)
	{
		...
		case contenedores.CANDIDATOS:
			template = "<div class='candidatosContainer'>"+
            	"<div class='title'>Candidatos a Jefe de Gobierno</div><div class='addCandidato' onclick='javascript:MostrarAgregarCandidato()'></div>"+
				"</div>";
			break;
...
function CompartirInicio(tipo)
{
	if(tipo == 0)
	  ...
	  'text='+ '%23yvosquepropones las propuestas de todos los pre-candidatos para CABA en un solo lugar queproponen.com.ar/PASO/', 'tweet', 'width=900,height=300,menubar=no,status=no,titlebar=no,top=200,left='+(screen.width-900)/2);
	else
	  window.open('http://www.facebook.com/dialog/feed?app_id=825676227513877' +
        '&link=http://queproponen.com.ar/PASO/' +
...
function HeaderPartido(part)
{
	...
	$(tweet).click(function(e) {
		window.open('https://twitter.com/intent/tweet?'+
		'url=http%3A%2F%2Fqueproponen.com.ar%2F'+'PASO%2F%23partido%2F'+part.nombre.split(' ').join('-')+'&'+
		'related=PartidodelaRed&'+
...
function HeaderCandidato(cand)
{
	...
	$(tweet).click(function(e) {
		window.open('https://twitter.com/intent/tweet?'+
		'url=http%3A%2F%2Fqueproponen.com.ar%2F'+'PASO%2F%23candidato%2F'+cand.nombre.split(' ').join('-')+'&'+
		'related=PartidodelaRed&'+
